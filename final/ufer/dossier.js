...
window.FWDOS = (function () {
  ...
  function zustand(jahr) {
    ...
    if (alter <= 3)  return "Neubau";
    if (alter <= 15) return "Neuwertig";
    if (alter <= 40) return "Gepflegt";
    return "Älterer Baubestand";
  }
  ...
  function bauen(l) {
    ...
    if (l.text || l.beschreibung) d.story = { titel:"Beschreibung", absaetze:[l.text || l.beschreibung] };
    ...
    if (profi) {
      d.dokumente = [{ name:"Objektdokumentation", typ:"pdf", zugang:"anfrage", hinweis:"Wird nach Ihrer Anfrage freigeschaltet" }];
      if (l.propertyType !== "grundstueck" && l.propertyType !== "parkplatz")
        d.dokumente.push({ name:"Grundriss", typ:"pdf", zugang:"anfrage" });
    }
